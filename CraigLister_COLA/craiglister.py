...
# gmailUser = "dml1002313@gmail.com"
# gmailPass = "SteveNash70!SteveNash70!"

gmailUser = "ohQuaMeu9oaz@gmail.com"
gmailPass = "ohQuaMeu9oaz#"

ClUSer = "ohQuaMeu9oaz@gmail.com"
ClPass = "SteveNash70!"
...
def chooseFirstOption(listing):
    listing.driver.find_element_by_name('n').click()
...
def clickListingType(listing):
    time.sleep(2)
    listing.driver.find_element_by_xpath("//li[10]").click()
    print listing.driver

def clickListingCategory(listing):
    time.sleep(2)
    listing.driver.find_element_by_xpath("//li[5]").click()
...
def fillOutListing(listing):
    ...
    listing.driver.find_element_by_id("GeographicArea").send_keys(listing.city + ", " + listing.state)
    ...
    listing.driver.find_element_by_id("contact_phone").send_keys("803-369-0134")
    listing.driver.find_element_by_id("contact_name").send_keys("Steve")
    listing.driver.find_element_by_id("contact_text_ok").click()
    listing.driver.find_element_by_id("contact_phone_ok").click()
    ...
    time.sleep(2)
    listing.driver.find_element_by_xpath("//button[1]").click()
...
def clickAcceptTerms(listing):
    listing.driver.find_element_by_xpath("//button[contains(.,'ACCEPT the terms of use')]").click()

def clickPublishListing(listing):
	listing.driver.find_element_by_xpath("//button[contains(.,'publish')]").click()

#need to write this function
def login(listing):
	listing.driver.find_element_by_id('inputEmailHandle').send_keys(ClUSer)
	listing.driver.find_element_by_id('inputPassword').send_keys(ClPass)
	time.sleep(2)
	listing.driver.find_element_by_css_selector('button.accountform-btn').click()
...
def postListing(listing):
    ...
    print " DONE: clickListingType(listing) "
    clickListingCategory(listing)
    print " DONE: clickListingCategory(listing) "
    clickAbideByGuidelines(listing)
    print " DONE: clickAbideByGuidelines(listing) "
    login(listing)
    print " DONE: login(listing) "
    # chooseFirstOption(listing)

    fillOutListing(listing)
    print " DONE: fillOutListing(listing) "

    #press continue when displays location
    listing.driver.find_element_by_css_selector('button.continue').click()
